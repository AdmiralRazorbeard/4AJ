...
{
	header('location:index.php?section=repas&semaineAnneFrank='.$_POST['semaineAnneFrank']);
}
if(!empty($_POST['semaineNobel']) && is_numeric($_POST['semaineNobel']))
{
	header('location:index.php?section=repas&semaineNobel='.$_POST['semaineNobel']);
}
	...
	// Si l'utilisateur a saisi des variables
{
		...
		// Si il a l'autorisation de réserver, on réserve
	{
		...
		if($residence == 1)
		{
			header('location:index.php?section=repas&semaineAnneFrank='.$semaineDuAnneFrank);
		}
		elseif($residence == 2)
		{
			header('location:index.php?section=repas&semaineClairLogis='.$semaineDuClairLogis);
		}
	}
